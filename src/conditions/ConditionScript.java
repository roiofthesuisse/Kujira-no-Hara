...
public class ConditionScript extends Condition implements CommandeEvent {
	...
	private static final String HYPOTHENUSAGE = HYPOTHENUSE_DEBUT + ESPACE + RELATIF + ESPACE + HYPOTHENUSE_MILIEU
			+ ESPACE + RELATIF + ESPACE + HYPOTHENUSE_FIN;
	...
	private static final String TOUCHE_C = "Input.trigger?(Input::C)";
	private static final String TOUCHE_X = "Input.trigger?(Input::X)";
	...
	public final boolean estVerifiee() {
		...
		// $game_map.events[@event_id].lolilol(nil, nil, 1) ||
		// $game_map.events[@event_id].lolilol(nil, nil, 3)

		// ($game_player.target_in_da_zone?(@event_id, 2) &&
		// $game_player.prout("Jiyounasu AttaqueEpee character", nil, 2)) ||
		// ($game_player.target_in_da_zone?(@event_id, 1) &&
		// $game_player.prout("Jiyounasu AttaqueTorche character", nil, nil)) ||
		// ($game_player.target_in_da_zone?(@event_id, 2) &&
		// $game_player.prout("Jiyounasu AttaqueEpee character", nil, 3))

		// Input.trigger?(Input::C)
		// ConditionScript : Input.trigger?(Input::X)

		// ! ($game_map.events[@event_id].event_arround?("An�mone HP[1] RESET", 50) ||
	...
	private String traiter(final String expression) {
		...
		if (m.find()) {
			...
			final int hypothenuse = ((int) Math
					.sqrt(nombres.get(0) * nombres.get(0) + nombres.get(1) * nombres.get(1)));
		...
		if (TOUCHE_X.equals(expression)) {
			return ToucheRole.MENU.enfoncee() ? "1" : "0";
		}
